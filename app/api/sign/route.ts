...
import { getEngagement, getEngagementPdfBytes } from "@/lib/get-engagement";
...
export async function POST(req: NextRequest) {
  ...
  if (!engagement.documentStoragePath) {
    return NextResponse.json(
      { error: "No signed-ready PDF has been uploaded for this client yet" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const pdfBytes = await getEngagementPdfBytes(
      engagement.documentStoragePath,
    );
