...
export async function buildSingleDocumentHtml(params: {
  clientName: string;
  ...
  const bodyHtml = await marked.parse(params.markdown);

  const docLabel = params.docType === "sow" ? "Statement of Work" : "Master Services Agreement";

  const html = `<!DOCTYPE html>
<html><head><meta charset="utf-8"><style>${css}</style></head>
<body>
  <div class="doc-title">${params.clientName} &times; Fonder</div>
  <div class="doc-subtitle">${params.engagementTitle} — ${docLabel}</div>
  ...
  const footerHtml = `<div style="width:100%; font-size:8pt; font-family: Arial, sans-serif; color:#8a8d93; text-align:right; padding: 0 0.5in; letter-spacing:0.03em;">
    ${params.clientName} &times; Fonder Studio — ${docLabel}
  </div>`;
