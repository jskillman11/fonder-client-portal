...
export function EngagementPortalContentForm({
  engagementId,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/update-engagement", {
      ...
      body: JSON.stringify({ engagementId, lockPortalTabs, tabLockOverrides }),
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <Card className="px-9 py-9">
        <h2 className="text-[16px] font-bold text-[var(--color-ink)] mb-1">Portal content</h2>
        ...
        <div className="mt-5">
          ...
          <p className="text-[11px] text-[var(--color-faint)] mt-1 mb-2">
            Overrides the lock above for a specific tab, regardless of whether documents have
            been sent yet.
          </p>
